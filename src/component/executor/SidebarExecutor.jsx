      ...
      <div className="flex-1 border-r overflow-auto py-2">
        <nav className="space-y-1 px-2">
          <div className="space-y-1">
            <NavItem href="/realisateur" icon={Home} label="Accueil" isActive={isActive("/realisateur")} collapsed={collapsed} />

            <Button
              variant="outline"
              size="sm"
              className={cn("mt-2 w-full gap-2 border-dashed h-[40px] border-blue-200 text-blue-600 hover:bg-blue-50 hover:text-blue-700", collapsed ? "justify-center px-2" : "justify-start")}
              onClick={() => navigate("/realisateur/create")}
            >
            ...
            {collapsed ? (
              ...
              <Collapsible open={openCategories.resources} onOpenChange={() => toggleCategory("resources")}>
                ...
                <CollapsibleContent className="space-y-1">
                  <NavItem href="/realisateur/services" icon={Layers} label="Services" isActive={isActive("/realisateur/services")} collapsed={collapsed} />
                  <NavItem href="/realisateur/historique" icon={FileText} label="Historique des demandes" isActive={isActive("/realisateur/historique")} collapsed={collapsed} />
            ...
            {collapsed ? (
              ...
              <Collapsible open={openCategories.management} onOpenChange={() => toggleCategory("management")}>
                ...
                <CollapsibleContent className="space-y-1">
                  ...
                  <NavItem href="/realisateur/messages" icon={MessageSquare} label="Messages" isActive={isActive("/realisateur/messages")} collapsed={collapsed} badge={unreadCount}/>
                </CollapsibleContent>
              </Collapsible>
            )}
          </div>
...
function NavItem({ href, icon: Icon, label, isActive, collapsed, badge }) {
  return (
    <Link to={href} className={cn("group flex items-center gap-3 rounded-md px-3 py-2 text-sm font-medium transition-colors", isActive ? "bg-blue-50 text-blue-700" : "text-gray-700 hover:bg-gray-100 hover:text-gray-900", collapsed && "justify-center px-2")}
    >
      <div className={cn("flex h-6 w-6 items-center justify-center rounded-md", isActive ? "bg-blue-100 text-blue-700" : "text-gray-500 group-hover:text-gray-700")}>
